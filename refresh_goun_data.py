...
GOUN_KEYMAP = {**KEYMAP, "입주유형": "moveIn"}
...
def main():
    if not SOURCE.exists():
        print(f"고운동 원본 파일 없음, 기존 고운동 데이터 유지: {SOURCE}")
        return
    ...
    if missing_sheets:
        raise ValueError(f"고운동 원본에 필요한 시트가 없습니다: {', '.join(missing_sheets)}")

    headers, source_rows = read_rows(workbook[LISTING_SHEET], GOUN_KEYMAP, listing_ids=True)
    ...
    print(
        f"고운동 {len(incoming_rows):,}건 반영{skipped_note}, "
