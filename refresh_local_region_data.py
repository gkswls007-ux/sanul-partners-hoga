...
LOCAL_KEYMAP = {**KEYMAP, "입주유형": "moveIn"}
...
def merge_listing_history(output, incoming_rows):
...
def refresh_region(region, source, output):
    if not source.exists():
        print(f"{region} 원본 파일 없음, 기존 {region} 데이터 유지: {source}")
        return
    ...
    if missing_sheets:
        raise ValueError(f"{region} 원본에 필요한 시트가 없습니다: {', '.join(missing_sheets)}")

    headers, source_rows = read_rows(workbook[LISTING_SHEET], LOCAL_KEYMAP, listing_ids=True)
    incoming_rows = [row for row in source_rows if is_valid_listing_row(row)]
    skipped_row_count = len(source_rows) - len(incoming_rows)
    rows = merge_listing_history(output, incoming_rows)
    ...
    print(
        f"{region} {len(incoming_rows):,}건 반영{skipped_note}, "
