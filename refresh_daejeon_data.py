...
SOURCE = Path(r"C:\Users\gkswl\OneDrive\바탕 화면\호가 및 실거래가_대전.xlsx")
OUTPUT = Path(__file__).parent / "data" / "listings-daejeon.json"
LISTING_SHEET = "매물요약"
BROKER_SHEET = "동일매물_중개사"
REAL_TRANSACTION_SHEET = "타입별_실거래가"
DAEJEON_KEYMAP = {**KEYMAP, "입주유형": "moveIn"}
...
def main():
    if not SOURCE.exists():
        print(f"대전 원본 파일 없음, 기존 대전 데이터 유지: {SOURCE}")
        return
    ...
    if missing_sheets:
        raise ValueError(f"대전 원본에 필요한 시트가 없습니다: {', '.join(missing_sheets)}")

    headers, incoming_rows = read_rows(workbook[LISTING_SHEET], DAEJEON_KEYMAP, listing_ids=True)
    ...
    print(f"대전 {len(incoming_rows):,}건 반영, 누적 {len(rows):,}건: {OUTPUT}")
